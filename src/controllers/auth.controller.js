...
class AuthController {
	async register(req, res) {
		try {
			...
			const hashedRefreshToken = bcrypt.hashSync(refreshToken.split("").reverse().join(""), 5);
	...
	async refresh(req, res) {
		try {
			...
			const isTokenCorrect = await bcrypt.compare(refreshToken.split("").reverse().join(""), user.refresh_token);
			...
			const hashedRefreshToken = bcrypt.hashSync(newRefreshToken.split("").reverse().join(""), 5);
